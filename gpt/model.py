...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att = torch.matmul(q, k.transpose(-2,-1)) / torch.sqrt(C // self.n_head)
        ...
        trunc_mask = self.casual_mask[:, :, :T, :T]
        # trunc_mask = self.casual_mask[1, 1, :T, :T]
        # trunc_mask = trunc_mask.expand(B, n_head, T, T)
        att = att.masked_fill(trunc_mask == 0, float('-inf'))

        # step 3: apply the softmax function to the masked attention matrix
        # hint: you can use the `F.softmax` function
        softmax_att = F.softmax(att, dim=-1)

        # step 4: apply the attention dropout (self.attn_dropout) to the attention matrix
        dropout_att = self.attn_dropout(softmax_att)

        # step 5: multiply the attention matrix with value vectors
        y = torch.matcul(dropout_att, v)
